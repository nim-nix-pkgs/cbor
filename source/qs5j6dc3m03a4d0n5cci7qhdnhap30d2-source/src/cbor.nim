...
import std/[endians, hashes, math, options, streams, strutils, tables, times, typetraits]
...
type
  ...
  CborParser* = object ## CBOR parser state.
    ...
    kind*: CborEventKind
    position*: int
      ## The initial stream position of the current item.
...
proc nextText*(c: var CborParser): string = nextText(c, result)
  ## Read the text that the parser is positioned on into a string and advance.
  ...
  of CborEventKind.cborBytes, CborEventKind.cborText:
    if c.isIndefinite:
      ...
      while c.kind != CborEventKind.cborBreak:
        ...
        let chunkLen = c.intVal.int
        c.s.setPosition(c.s.getPosition + chunkLen)
        c.next()
    else:
        c.s.setPosition(c.s.getPosition + c.intVal.int)
        c.next()
...
proc isSorted(n: CborNode): bool =
  ...
  for key in n.map.keys:
    ...
    if lastRaw != "":
      if thisRaw.len > lastRaw.len: return false
